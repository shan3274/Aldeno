...
export const FirebaseProvider = (props) => {
  ...
  const handleCreateNewListing = async (
    ...
    const imagRef = ref(
      storage,
      `uploads/${productid}Image/'${Date.now()}-${coverPic.name}`
    );
  ...
  const resellData = async (productid, productType, email, images, types) => {
    ...
    for (const image of images) {
      const imagRef = ref(
        storage,
        `Resell/${productid}Image/'${Date.now()}-${image.name}`
      );
      imageURLs[ins] = await uploadBytes(imagRef, image);
      ins++;
    }
    return await addDoc(
      collection(firestore, "Resell/2UHVmRt1JSAGgM0oUeil/products"),
      {
        productid,
        productType,
        email,
  ...
  const addProduct = async (
    ...
    for (const image of images) {
      const imagRef = ref(
        storage,
        `Product/${productid}Image/'${Date.now()}-${image.name}`
      );
      imageURLs[ins] = await uploadBytes(imagRef, image);
      ins++;
    }
    return await addDoc(
      collection(firestore, `products/bo87k2u9CqCxjHcV9RgS/${productid}`),
      {
        productid,
        productName,
        size,
  ...
  const Billing2 = async (productid, storeName, location, amount, date) => {
    return await addDoc(
      collection(firestore, "Traceability/2y37xdYTaS5EQdydxtAe/" + productid),
      {
        productid,
        storeName,
        location,
  ...
  const getList = (id) => {
    return getDocs(
      collection(firestore, `products/bo87k2u9CqCxjHcV9RgS/${id}`)
    );
  };
  ...
  const getTrace = (id) => {
    return getDocs(
      collection(firestore, "Traceability/2y37xdYTaS5EQdydxtAe/" + id)
    );
  };
